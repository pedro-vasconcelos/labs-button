...
export default defineComponent({
    ...
    methods: {
        resolveVariants(baseClass, variant) {
            if ( variant !== undefined ) {
                let className = variant.trim().replace('-', ' ').split(' ')
                className = className.map((word) => word[0].toUpperCase() + word.substring(1)).join('')
                return baseClass + className
            } else {
                return baseClass
            }
        },
        resolveClass(field: string, defaultValue: string) {
// const debugField = 'root_Class';
...
// (field === debugField) ? console.log('context', context) : null;
                currentClass = currentClass(context);
// (field === debugField) ? console.log('currentClass: ', currentClass) : null;
            } //else {
// (field === debugField) ? console.log('suffix: ', suffix) : null;
...
// (field === debugField) ? console.log('globalClass: ' + globalClass) : null;
            if (typeof globalClass === "function") {
                globalClass = globalClass(context || _getContext(this));
            } else {
